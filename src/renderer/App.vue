<template>
  <div
    class="background-container fixed inset-0 z-[-1] pointer-events-none"
    style="background: linear-gradient(135deg, #e6e9f0 0%, #eef1f5 100%)"
  >
    <div
      class="shape shape1 absolute"
      style="
        width: 350px;
        height: 350px;
  ...
  <div class="flex h-screen w-full relative">
    ...
    <!-- Main Content -->
    <main
      class="flex-grow h-screen overflow-y-auto transition-all duration-300 ease-in-out p-4 sm:p-6 lg:p-8"
    >
      <div class="w-full max-w-4xl mx-auto relative z-10">
        <header class="flex items-center justify-center mb-8">
          <div class="w-12 h-12 mr-4 flex items-center justify-center">
            <img src="./assets/tempLogo.png" alt="ClariFact Logo" class="w-12 h-12" />
          </div>
        ...
        <AnswerCard
          ...
          :summary="selectedData ? selectedData.summary : ''"
          :status="selectedData ? '불러오기 완료' : ''"
          :statusClass="selectedData ? 'bg-gray-100/50 text-gray-800' : ''"
        >
          <template #main-content-area>
            <div
              v-if="
                selectedData &&
                chatSessions.find((s) => s.id === activeSessionId).mode === 'comparison'
              "
            >
              <div class="overflow-x-auto">
                <table class="w-full text-left">
                  <thead class="bg-white/20">
                    <tr>
                      <th class="p-3 font-semibold text-sm border-b border-white/30">모델명</th>
                      <th class="p-3 font-semibold text-sm border-b border-white/30">핵심 사양</th>
                      <th class="p-3 font-semibold text-sm border-b border-white/30">특징</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div
              v-else-if="
                selectedData &&
                chatSessions.find((s) => s.id === activeSessionId).mode === 'knowledge'
              "
            >
            ...
            <div
              v-else-if="
                selectedData &&
                chatSessions.find((s) => s.id === activeSessionId).mode === 'professional'
              "
            >
              <div class="prose max-w-none">
                ...
                ><code>{{ selectedData.code }}</code></pre>
              </div>
            </div>
          ...
          <template #action-buttons>
            <button
              v-if="selectedData && selectedData.citation"
              class="px-3 py-1 text-sm bg-white/40 text-gray-700 rounded-full"
            >
              인용 복사 (APA)
            </button>
          </template>
          <template #source-list>
            <li
              v-for="(s, i) in selectedData ? selectedData.sources : []"
              :key="i"
          ...
          <template #verified-time>
            <span class="text-xs text-gray-500">{{
              selectedData ? new Date().toLocaleString('ko-KR') + ' 기준으로 생성됨' : ''
            }}</span>
          </template>
        </AnswerCard>
  ...
  const chatSessions = ref([
    {
      ...
      data: {
        title: '노트북 비교 (데모)',
        summary: '데모 데이터입니다. 실제 AI는 더 정확한 정보를 제공합니다.',
        products: [
          {
            name: '갤럭시북 Pro (데모)',
            specs: 'Intel Core i7, 16GB RAM',
            features: 'AMOLED 디스플레이, 초경량',
          },
          {
            name: 'LG 그램 (데모)',
            specs: 'Intel Core i5, 16GB RAM',
            features: '매우 가벼운 무게, 긴 배터리',
          },
          {
            name: '맥북 에어 (데모)',
            specs: 'Apple M3, 8GB RAM',
            features: 'macOS, 뛰어난 성능 효율',
          },
        ],
        sources: ['다나와 (샘플)', '노트북리뷰 (샘플)'],
      },
    },
    {
      id: 2,
      ...
      mode: 'knowledge',
      data: {
        title: '스태그플레이션 원인과 영향 (데모)',
        summary: '스태그플레이션은 경기 침체와 물가 상승이 동시에 발생하는 현상입니다.',
        sections: [
          {
            heading: '주요 원인',
            content:
              '1. 공급 충격: 원유 가격 급등 등으로 발생합니다.\n2. 정책 실패: 통화정책의 부작용 등.',
          },
          {
            heading: '거시 경제에 미치는 영향',
            content: '실업률과 물가 상승이 동시에 발생하여 가계의 실질 소득이 감소합니다.',
          },
        ],
        sources: ['한국은행 (샘플)', 'KDI (샘플)'],
        citation: '한국은행. (2025). 스태그플레이션. 경제용어사전.',
      },
    },
  ]);
